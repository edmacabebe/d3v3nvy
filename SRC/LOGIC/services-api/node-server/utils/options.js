...
module.exports = function() {
  ...
  var options = {
    ...
    mlManagePort: config.marklogic.managePort,
    mlSessionConfig:  {
      host: config.marklogic.host,
      port: config.marklogic.sessionPort,
      user: config.marklogic.sessionUser,
      password: config.marklogic.sessionPassword
    },
    ...
    guestAccess: bool(process.env.GUEST_ACCESS || envJson['guest-access'] ||
      config.marklogic.guestAccess || false),
    disallowUpdates: bool(process.env.DISALLOW_UPDATES || envJson['disallow-updates'] ||
      config.marklogic.disallowUpdates || false),
    appUsersOnly: bool(process.env.APP_USERS_ONLY || envJson['appusers-only'] ||
      config.marklogic.appUsersOnly || false)
  };
  ...
  function getEnvOptions(env) {
    ...
    try {
      ...
      envJson = {};
      console.log('Couldn\'t find ' + envFile +
        '; you can create this file to override properties - ' +
        '`gulp init-local` creates local.json which can be ' +
        'modified for other environments as well');
    }
